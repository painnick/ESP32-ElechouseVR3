...
/**
   ...
   0x0n:System buf[1]  -->  number of record which is recognized. buf[2]  -->
   Recognizer index(position) value of the recognized record. buf[3]  -->
   Signature length buf[4]~buf[n] --> Signature timeout --> wait time for
   receiving packet.
...
/**
        ...
        @param records --> record data buffer pointer.
           ...
           buf --> pointer of return value buffer, optional.
                   buf[0]  -->  number of records which are trained
   successfully. buf[2i+1]  -->  record number buf[2i+2]  -->  record train
   status. 00 --> Trained FE --> Train Time Out FF --> Value out of range" (i =
   0 ~ len-1 )
   ...
   0 ~ len-1 )
        @retval '>0' --> length of valid data in buf.
            ...
            '<0' --> failed.
                ...
                -2 --> train timeout.
*/
int VR ::train(uint8_t record, uint8_t *buf) { return train(&record, 1, buf); }
...
/**
        ...
        @param record --> record value.
           buf --> signature string/data pointer.
                   len --> lenght of buf.
                   retbuf --> return data .
                         retbuf[0]  -->  number of records which are trained
   successfully. retbuf[1]  -->  record number. retbuf[2]  -->  record train
   status. 00 --> Trained F0 --> Trained, signature truncate FE --> Train Time
   Out FF --> Value out of range" retbuf[3] ~ retbuf[retval-1] -->
   Signature.(retval means return value)
        @retval '>0' --> length of valid data in buf.
            0 --> success, and no data received.
            '<0' --> failed.
                -1 --> data format error.
...
/**
    ...
    @retval  0 --> success
            -1 --> failed
*/
int VR ::deleteSignature(uint8_t record) { return setSignature(record); }
...
/**
    ...
    @param buf --> return value buffer.
             ...
             buf[i+1]   -->  Record number.(0xFF: Not loaded(Nongroup mode), or
   not set (Group mode)) (i= 0, 1, ... 6) buf[8]     -->  Number of all voice
   records in recognizer buf[9]     -->  Valid records position indicate.
             buf[10]    -->  Group mode indicate(FF: None Group, 0x8n: User,
   0x0n:System
...
/**
    ...
    @param buf --> return value
             ...
             buf[2i+1]  -->  Record number.
             buf[2i+2]  -->  Record train status. (00: untrained, 01: trained,
   FF: record value out of range) (i = 0 ~ buf[0]-1 )
...
/**
    ...
    @param buf --> return value
            ...
            buf[4] --> Group control by external IO(0-disable 1-system group
   2-user group)
    @retval '>0' --> buf length
            -1 --> failed
*/
...
/**
    ...
    @retval 0 --> success
            -1 --> failed
*/
int VR ::disableAutoLoad() { return setAutoLoad(); }
...
/**
    @brief receive a valid data packet in Voice Recognition module protocol
   format.
    @param buf --> return value buffer.
